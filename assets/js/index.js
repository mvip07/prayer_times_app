...
//         <td>${i?.subject}</td>
//         <td>${i?.age}</td>
//         <td><button class="btn btn-warning" onclick="deleteStudent(${i?.id})"><i class="fa-solid fa-pen-to-square me-2"></i>Edit</button></td>
//         <td><button class="btn btn-danger" onclick="deleteStudent(${i?.id})"><i class="fa-solid fa-trash-can me-2"></i>Delete</button></td>
//       `;
...
//   let dataLen = data[data.length - 1];
//   let studentObj = { id: dataLen && dataLen.id > 0 ? dataLen.id + 1 : 1 };
...
// //   let deleteId = [];
//   let deleteId =  data?.map((j) => (j.id != id ? j : ""));

//   localStorage.setItem("student", JSON.stringify(deleteId))
